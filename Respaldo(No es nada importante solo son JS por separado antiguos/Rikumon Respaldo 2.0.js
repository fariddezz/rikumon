...
window.addEventListener('load', () =>
{
 ...
 inputThornhydro = document.getElementById('Thornhydro')
    ...
    function showAttacks(B){
        B.forEach((element) => {
            mokeponsAttacks = `<button id = ${element.id} class = "attack-button BAttack">${element.name} </button>`
        ...
        bElectro.addEventListener('click', attackElectro);
        bPyro.addEventListener('click', attackPyro);
        bHydro.addEventListener('click', attackHydro);
    }
    ...
    function checkedPet(){
        ...
        enemysPetVerification()

        extractAttacks(petselected)

        }

        //Invocacion de la funcion de seleccionar mascota
        checkedPet()
        ...
        function enemysPetVerification(){
            if(petselected == null){
        ...
        function ramdonEnemyAttack(){
            ...
            if(enemyAttack == 1){
                ...
                alert("I do error, sorry")
            }
            createMessage();
        }

    //Funcion de seleccion de Ataque        
        function attackElectro(){
            if(petselected != null){
            playerAttack = electro
            ramdonEnemyAttack()
            }
        }
